...
// arg3 is the gradation parameter (0 is uniform, higher values give more and
// more importance
...
//			default value : 10
// -np x : sets the number of wanted processes (useful only with
...
int main(int argc, char* argv[])
{
    ...
    typedef vtkDiscreteRemeshing<vtkIsotropicMetricForClustering>
        IsotropicRemeshing;
    ...
    typedef vtkDiscreteRemeshing<vtkQuadricAnisotropicMetricForClustering>
        QuadricAnisotropicRemeshing;
    ...
    if (argc > 1) {
        ...
        cout << "Usage : AnisotropicRemeshingQ file nvertices gradation "
                "[options]"
             << endl;
    ...
    while (ArgumentsIndex < argc) {
        ...
        if (strcmp(argv[ArgumentsIndex], "-q") == 0) {
            ...
            Remesh->GetMetric()->SetQuadricsOptimizationLevel(
                atoi(argv[ArgumentsIndex + 1]));
        }

        if (strcmp(argv[ArgumentsIndex], "-b") == 0) {
            cout << "Setting boundary fixing to : " << argv[ArgumentsIndex + 1]
                 << endl;
