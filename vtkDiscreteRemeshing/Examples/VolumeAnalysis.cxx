...
public:
    ...
    char* GetOutputFormat() { return (this->OutputFormat); }
    ...
    ~MyThreaderHelperClass() { this->Lock->Delete(); }
};
    ...
    for (int i = MyId; i < Labels->GetNumberOfIds(); i += NumberOfThreads) {
        ...
        if (Helper->NamesMap[Level].length() > 0) {
            Name << Helper->OutputDirectory << Level << "-"
                 << Helper->NamesMap[Level] << "." << Helper->GetOutputFormat();
        } else {
            Name << Helper->OutputDirectory << Level << "."
                 << Helper->GetOutputFormat();
        }
        int MaxNumberOfVertices = Helper->MaximumNumberOfVertices;
        if ((Mesh->GetNumberOfPoints() > MaxNumberOfVertices) &&
            (MaxNumberOfVertices > 0)) {
            int WantedNumberOfVertices = MaxNumberOfVertices;
            Timer->StartTimer();
            if (Helper->SimplificationType == 1) {
                vtkQuadricClustering* Simplification =
                    vtkQuadricClustering::New();
                Simplification->SetInputData(Mesh);
                int NumberOfSubdivisions =
                    (int)pow((double)WantedNumberOfVertices, (double)1.0 / 3.0);
                Simplification->SetNumberOfDivisions(
                ...
                if (Helper->Anisotropy != 0) {
                    WantedNumberOfIsotropicVertices =
                        30 * WantedNumberOfVertices;
                }
                ...
                if (1) {
                    ...
                    double** ClustersQuadrics =
                        new double*[WantedNumberOfIsotropicVertices];
                    ...
                    for (int i = 0; i < Remesh->GetNumberOfItems(); i++) {
                        ...
                        if ((Cluster >= 0) &&
                            (Cluster < WantedNumberOfIsotropicVertices)) {
                            if (Remesh->GetClusteringType() == 0) {
                                vtkQuadricTools::AddTriangleQuadric(
                                    ClustersQuadrics[Cluster],
                                    Remesh->GetInput(), i, false);
                            } else {
                                Remesh->GetInput()->GetVertexNeighbourFaces(
                                    i, FList);
                                for (int j = 0; j < FList->GetNumberOfIds();
                                     j++)
                                    vtkQuadricTools::AddTriangleQuadric(
                                        ClustersQuadrics[Cluster],
                                        Remesh->GetInput(), FList->GetId(j),
                                        false);
                            }
                        } else
                ...
                if (Helper->Anisotropy != 0) {
                    vtkAnisotropicDiscreteRemeshing* AnisoRemesh =
                        vtkAnisotropicDiscreteRemeshing::New();
...
int main(int argc, char* argv[])
{
    ...
    if (argc < 2) {
        ...
        cout << "Options : " << endl;
        cout << "-n number : sets the number of simplified meshes vertices "
                "(default: 32000)"
             << endl;
        cout << "-v number : sets the maximum number of voxels before the "
                "image gets resampled (default : 100000000)"
             << endl;
        cout << "-r number : sets the maximum subsampling factor (default : 4)"
             << endl;
        cout << "-j number : sets the number of threads (default : auto)"
             << endl;
        cout << "-s number : sets the simplification method (default: 0 : ACVD)"
             << endl;
        cout << "-sm number : sets the number of smoothing steps (default : 0)"
             << endl;
        cout << "-g number : sets the gradation parameter when using ACVD"
             << endl;
        cout << "-o directory : sets the output directory" << endl;
        cout << "-f format : sets the output mesh format (default : vtk)"
             << endl;
        cout << "-m 0/1 : forces manifold output (default : 0)" << endl;
        cout << "-a 0/1 : uses anisotropic coarsening (default : 0)" << endl;
        cout << "-c 0/1 : keeps (or not) only each biggest component (default "
                ": 0)"
             << endl;
        ...
        if (strcmp(argv[ArgumentsIndex], "-x") == 0) {
            ...
            for (int j = 0; j < RootElement->GetNumberOfNestedElements(); j++) {
                ...
                if (strcmp(XMLColors->GetName(), "colors") == 0) {
                    for (int i = 0; i < XMLColors->GetNumberOfNestedElements();
                         i++) {
                        vtkXMLDataElement* Element =
                            XMLColors->GetNestedElement(i);
                        if (strcmp(Element->GetName(), "color") == 0) {
                            if ((Element->GetAttribute("label") != 0) &&
                                ...
                                ColorMap[Label] =
                                    Element->GetAttribute("meshcolor");
                                cout << "Label " << Label << " has color ["
                                     << Element->GetAttribute("meshcolor")
                                     << "]" << endl;
                            }
                            if ((Element->GetAttribute("label") != 0) &&
                                ...
                                int Label;
                                Element->GetScalarAttribute("label", Label);
                                Helper.NamesMap[Label] =
                                    Element->GetAttribute("name");
                                cout << "Label " << Label << " has name : "
                                     << Element->GetAttribute("name") << endl;
                            }
                        }
                    }
                    cout << endl;
    ...
    int Dimensions[3];
    Image->GetDimensions(Dimensions);
    cout << "Image dimensions : " << Dimensions[0] << " " << Dimensions[1]
         << " " << Dimensions[2] << endl;
    ...
    for (int i = 0; i < Labels->GetNumberOfIds(); i++) {
        ...
        if (Helper.NamesMap[Label].length() > 0) {
            Name << Label << "-" << Helper.NamesMap[Label] << "."
                 << Helper.GetOutputFormat();
        } else {
        ...
        if (Color.length() == 0)
            cout << "Warning : no color was provided for label " << Label
                 << endl;
        else
