...
=========================================================================*/
// .NAME vtkOOCMetaImageReader - Superclass of transformable binary file
// readers. .SECTION Description vtkOOCMetaImageReader provides methods needed
// to read a region from a mhd file. It supports both transforms and masks on
// the input data, but as a result is more complicated and slower than its
// parent class vtkImageReader2.
...
public:
    ...
    virtual const char* GetFileExtensions() { return ".mhd .mha"; }
    ...
    /**
    ** Additional methods required for extracting all the information that's
    *contained within
    ** .mhd files
    **/
    ...
    /**
     * Additional macros required for extracting all the information that's
     *contained within .mhd files
     **/
...
protected:
    ...
    // This method is not overloaded in vtkImageReader; it is, however, in
    // vtkMetaImageReader (quite predictably, it must be said). Hence, it shall
    // replicate the latter method.
    void ExecuteInformation();

    // Here, the .mhd file information is requested by the reader.
    // As such, this method has to duplicate the behavior of the
    // vtkMetaImageReader Nonetheless, it is important to note that the
    // implementations contained within vtkImageReader and vtkMetaImageReader
    // difer by more than small details. As such, a middle-ground must be found
    // between both
    ...
    // This is the method where the actual reading of the file takes places. As
    // such, it should replicate vtkImageReader's functionality because it's
    // this class that permets the lecture of regions of interest
