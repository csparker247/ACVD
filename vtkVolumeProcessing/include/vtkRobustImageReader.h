...
public:
    ...
    {
        vtkObject* ret =
            vtkObjectFactory::CreateInstance("vtkRobustImageReader");
        if (ret) {
    ...
    void Update()
    {
        ...
        vtkImageReader2* Reader =
            imageReaderFactory->CreateImageReader2(FileName);

        if (!Reader) {
            std::cerr << "Cannot load file " << FileName
                      << " as an image file; terminating.\n";
            exit(5);
        }
        ...
        if (strcmp(Reader->GetClassName(), "vtkMetaImageReader") == 0) {
            ...
            while (is) {
                ...
                if ((prefix == "TransformMatrix") ||
                    (prefix == "Orientation") || (prefix == "Rotation")) {
                    matrixPrefix = prefix;
        ...
        for (int i = 0; i < 3; i++) {
            if (!flip[i])
                continue;
            std::cout << "Warning : RobustReader flipping dimension " << i
                      << std::endl;
    ...
    vtkGetObjectMacro(Output, vtkImageData)
        ...
        vtkGetMacro(FileName, char*) vtkSetMacro(FileName, char*)

            protected : vtkImageData* Output;
