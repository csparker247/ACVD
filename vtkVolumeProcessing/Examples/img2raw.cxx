...
void printSyntax(ostream& os)
{
    ...
    os << "from input images and/or directories of images specified in any "
          "order.\n";
    os << "\t* Supported image formats:\n";
    os << "\t\tPNG, PNM, TIFF, BMP, SLC, JPEG, GE Signa, MINC, Meta Image (MHD "
          "+ RAW)\n";
    os << "\t* Ouput volume is of dimensions and voxel data type as to "
          "accommodate all input.\n";
...
int main(int argc, char* argv[])
{
    ...
    vector<string>::iterator itOptO =
        find(arguments.begin(), arguments.end(), "-o");
    ...
    if (pointPos < outputFileName.length()) {
        string outputFileExtension =
            outputFileName.substr(pointPos + 1, outputFileName.length());
    ...
    vector<string>::iterator itOptC =
        find(arguments.begin(), arguments.end(), "-c");
    ...
    vector<string>::iterator itProg =
        find(arguments.begin(), arguments.end(), argv[0]);
    ...
    vtkSmartPointer<vtkImageReader2Factory> imageReaderFactory =
    ...
    for (int i = 0; i < arguments.size(); ++i) {
        ...
        if (directory->FileIsDirectory(arguments[i].c_str())) {
            if (!directory->Open(arguments[i].c_str())) {
                cerr << "Cannot open directory " << arguments[i]
                     << "; skipping it.\n";
                continue;
            }
            ...
            for (int j = 0; j < numberOfFiles; ++j) {
                if ((string(".") == directory->GetFile(j)) ||
                    (string("..") == directory->GetFile(j)))
                    continue;
                string fileName =
                    arguments[i] + string("/") + directory->GetFile(j);
                vtkSmartPointer<vtkDirectory> auxDirectory =
                    vtkSmartPointer<vtkDirectory>::New();
                if (auxDirectory->FileIsDirectory(fileName.c_str())) {
                    cerr << "Found sub-directory " << fileName
                         << "; skipping it.\n";
                    continue;
                }
            ...
            for (int j = 0; j < fileNames.size(); ++j) {
                vtkImageReader2* imageReader =
                    imageReaderFactory->CreateImageReader2(
                        fileNames[j].c_str());
                if (!imageReader) {
                    cerr << "Cannot load file " << fileNames[j]
                         << " as an image file; skipping it.\n";
                    continue;
                } else {
            ...
            vtkImageReader2* imageReader =
                imageReaderFactory->CreateImageReader2(arguments[i].c_str());
            if (!imageReader) {
                cerr << "Cannot load file " << arguments[i]
                     << " as an image file; skipping it.\n";
                continue;
            } else {
        ...
        cout << '\r' << "Reading input... " << setw(3)
             << static_cast<int>(
                    100.0 * (static_cast<double>(i + 1) / arguments.size()))
             << "% ";
        cout.flush();
    }
    ...
    volumeImageData->SetExtent(
        0, volumeWidth - 1, 0, volumeHeight - 1, 0, volumeDepth - 1);
    ...
    int size = volumeImageData->GetScalarSize() * dimensions[0] *
               dimensions[1] * volumeImageData->GetNumberOfScalarComponents();

    for (int i = 0; i < imageReaders.size(); ++i) {
        vtkImageData* imageData = imageReaders[i]->GetOutput();
        cout << '\r' << "Constructing volume... " << setw(3)
             << static_cast<int>(
                    100.0 * (static_cast<double>(i) / imageReaders.size()))
             << "% ";
        cout.flush();
    ...
    // Write out volume
    vtkSmartPointer<vtkMetaImageWriter> volumeWriter =
    ...
    cout << "\tVolume dimensions: width = " << volumeWidth
         << ", height = " << volumeHeight << ", depth = " << volumeDepth
         << '\n';
    cout << "\tVoxels have " << volumeScalarComponents << " components of size "
         << volumeScalarSize << " bytes.\n";
}
