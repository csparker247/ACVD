...
Program:   VolumeAnisotropicDiffusion : processes median filtering on input
volume Module:    vtkSurface Language:  C++ Date:      2011/09 Auteur: Sebastien
Valette
...
{
    if (argc < 3) {
        cout << "Usage : VolumeAnisotropicDiffusion file.mhd "
                "numberofiterations []"
             << endl;
        exit(1);
    }
    ...
    while (ArgumentsIndex < argc) {
        if (strcmp(argv[ArgumentsIndex], "-t") == 0) {
            ...
            cout << "Diffusion threshold=" << atof(argv[ArgumentsIndex + 1])
                 << endl;
        }

        if (strcmp(argv[ArgumentsIndex], "-f") == 0) {
            Diffusion->SetDiffusionFactor(atof(argv[ArgumentsIndex + 1]));
            cout << "Diffusion factor=" << atof(argv[ArgumentsIndex + 1])
                 << endl;
        }
        ArgumentsIndex += 2;
    }
