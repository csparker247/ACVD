...
void ResampledVolumeGenerator::AssignElementDataFile ( ){
    ...
    // std::cout << "Delete this shit, element data name " << _elementDataName
    // << std::endl;
}
...
void ResampledVolumeGenerator::GetImageInformation ( ){
    ...
    if (_write) {
        ...
        writer->SetImageDimensions(
            _resampledDimensionX, _resampledDimensionY, _resampledDimensionZ);
...
void ResampledVolumeGenerator::StartWork ( ){
    ...
    std::cout << "We're in for "
              << _divisionFactorX * _divisionFactorY * _divisionFactorZ
              << " iterations" << std::endl;
    ...
    for (int z = 0; z < _divisionFactorZ; z++) {
        for (int x = 0; x < _divisionFactorX; x++) {
            for (int y = 0; y < _divisionFactorY; y++) {
                ...
                iteration++;
                // ENCORE A FAIRE: Trouver une mani�re pour ne pas devoir cr�er
                // un nouveau reader/image � chaque fois ENCORE A FAIRE: Il y a
                // quelque chose d'inefficient . J'utilise une instance du reader
                // pour obtenir les param�tres de l'image, et de nombreuses
                // autres instances pour lire les subvolumes. Ce n'est pas tr�s
                // intelligent, est-ce qu'il n'existe pas une mani�re plus
                ...
                // ENCORE A FAIRE: Trouver une mani�re pour ne pas devoir cr�er
                // un nouveau resampler/image � chaque fois Voil�, quelqu'un
                // d'autre est en charge du sous-�chantillonnage!
                ...
                // ENCORE A FAIRE: Comment faire pour utiliser la m�me instance
                // du reader pour obtenir la metadata et les donn�es du fichier?
                // ENCORE A FAIRE: �crire ligne par ligne et pas pixel par pixel
                // En fait, tout ce qui vient apr�s devrait se faire dans une
                // autre classe que soit en charge de l'�criture dans le fichier!
                ...
                // ENCORE A FAIRE: Il faut bien regarder o� l'on est en train de
                // demander de la m�moire, ici et ailleurs. Effectivement, la
                // cr�ation constante des nouvelles classes fait p�ter le truc.
                // C'est normal, les images de S�bastien sont g�antes. Du coup,
                // il faut faire gaffe.
                reader->Delete();
                resampler->Delete();
                sousVolume->Delete();
...
char* ResampledVolumeGenerator::GetNewMHDFileName() { return _mhdFilename; }
