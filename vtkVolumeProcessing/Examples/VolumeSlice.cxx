...
VTK_THREAD_RETURN_TYPE ThreadedImageSlice (void *arg) {
    ...
    Slicer->SetResliceAxesDirectionCosines(
        params[0][0], params[0][1], params[0][2], params[1][0], params[1][1],
        params[1][2], params[2][0], params[2][1], params[2][2]);
    ...
    for (int i = MyId; i < NumberOfSlices; i += NumberOfThreads) {
        ...
        if (Image->GetNumberOfScalarComponents() > 1) {
            ...
            if (Helper->Format == 0) {
                ...
                unsigned char* PNGPointer =
                    (unsigned char*)PNGSlice->GetScalarPointer();
...
int main( int argc, char *argv[] )
{
    ...
    if (argc < 2) {
        ...
        cout << "-mhdcolors : create png image without using scale nor shifting"
             << endl;
        cout << "-orientation : set the axe used for the orientation of the "
                "slice (0 : z; 1 : x; 2 : y) default : 0"
             << endl;
        exit(1);
    }
    ...
    // Keep transformation matrix and anatomical orientation of input meta
    // image, it needs to be copied to output images. Since VTK currently offers
    // no methods to read and write this information, we're forced to use IO
    // streams
    ...
    if (imageReaderType == "vtkMetaImageReader") {
        ...
        while (is) {
            ...
            iss >> prefix;
            if ((prefix == "TransformMatrix") || (prefix == "Orientation") ||
                (prefix == "Rotation")) {
                matrixPrefix = prefix;
    ...
    // note : the * 3 multiplicator bellow is an attempt at reducing
    // segfaults...
    ...
    XMLRange->SetIntAttribute(
        "numberOfScalarComponents", Image->GetNumberOfScalarComponents());
    ...
    if (anOrientation) {
        ...
        XMLAnOr->SetCharacterData(
            Helper.anatomicalOrientation,
            strlen(Helper.anatomicalOrientation) + 1);
