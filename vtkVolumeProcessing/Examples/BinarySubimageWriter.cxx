...
// ENCORE A FAIRE: Bien définir les destructeurs de mes classes afin d'y inclure
// les 'deletes' de chaque élement
    ...
    for (int zSV = ezi; zSV <= ezf; zSV++) {
        for (int ySV = eyi; ySV <= eyf; ySV++) {
            for (int xSV = exi; xSV <= exf; xSV++) {
                // std::cout << "X " << xSV << " Y " << ySV << " Z " << zSV <<
                // std::endl;
                char* pixel = (char*)(_image->GetScalarPointer(xSV, ySV, zSV));
                // std::cout << "Pixel value " << pixel << std::endl;
                int position =
                    (xSV + ySV * _imageDimensions[0] +
                     zSV * (_imageDimensions[0] * _imageDimensions[1])) *
                    (_pixelSize)*_numberOfChannels /*_pixelSize*/;
                // std::cout << "Pixel Value " << (unsigned short*)pixel <<
                // std::endl;
...
{
    std::cout << "The file that is going to be opened " << _Filename
              << std::endl;
    binary_file.open(_Filename, fstream::out | fstream::binary);
}
void BinarySubimageWriter::CloseFile() { binary_file.close(); }