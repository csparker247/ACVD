...
template <class T>
void vtkImageDataCleanLabelsExecute(
    vtkImageData* input, vtkImageData* output, T* inPtr, T* outPtr)
{
    int dimensions[3];
            ...
            while (!voxelsToVisit.empty()) {
                ...
                if ((outPtr[pointer] == 0) &&
                    ...
                    if (x1 != 0) {
                        ...
                        if ((outPtr[neighbourPointer] == 0) &&
                            (neighbourLabel == currentLabel)) {
    ...
    /*	for (unsigned int i=0;i<components.size();i++) {
            cout<<"Component : "<<i<<" : label="<<(int) components[i].label <<",
       size="<<components[i].size<<endl; typename std::map<T , unsigned
       int>::iterator it; cout<<"   neighbours : "; for (
       it=components[i].neighbours.begin() ; it !=
       components[i].neighbours.end(); it++ )
            {
    ...
    for (unsigned int i = 0; i != labels.size(); i++) {
        ...
        if (currentLabelComponents.size() > 1) {
            cout << "label " << i << " has " << currentLabelComponents.size()
                 << " components" << endl;
    ...
    while (componentsToRemove.size()) {
        ...
        std::map<T, unsigned int> neighbours =
            components[componentToRemove].neighbours;
        ...
        while (!voxelsToVisit.empty()) {
            ...
            if ((outPtr[pointer] == unusedLabel) &&
                ...
                if (x1 != 0) {
                    neighbourPointer = pointer - 1;
                    neighbourLabel = inPtr[neighbourPointer];
                    if ((outPtr[neighbourPointer] == unusedLabel) &&
                        (neighbourLabel == currentLabel)) {
                        neighbourVoxel.pointer = neighbourPointer;
                        neighbourVoxel.x = x1 - 1;
                        neighbourVoxel.y = y1;
...
void vtkImageDataCleanLabels::SimpleExecute(
    vtkImageData* input, vtkImageData* output)
{
    void* inPtr = input->GetScalarPointer();
    void* outPtr = output->GetScalarPointer();
    switch (input->GetScalarType()) {
        vtkTemplateMacro(vtkImageDataCleanLabelsExecute(
            input, output, (VTK_TT*)(inPtr), (VTK_TT*)(outPtr)));
        default:
            vtkGenericWarningMacro(
                "ThreadedRequestData: Unknown output ScalarType");
            return;
    }
}
