...
void vtkOOCMetaImageReader::ReadImageParameters()
{
    ...
    switch (this->MetaImagePtr->ElementType()) {
        default:
            vtkErrorMacro(
                << "Unknown data type: " << this->MetaImagePtr->ElementType());
            return;
    ...
    vtkmetaio::MET_SizeOfType(
        this->MetaImagePtr->ElementType(), &BitsAllocated);
...
int vtkOOCMetaImageReader::OpenAndSeekFile(int dataExtent[6], int idx)
{
    ...
    if (this->FileLowerLeft) {
        streamStart = streamStart + (dataExtent[2] - this->DataExtent[2]) *
                                        this->DataIncrements[1];
    } else {
        streamStart = streamStart + (this->DataExtent[3] - this->DataExtent[2] -
                                     dataExtent[2]) *
                                        this->DataIncrements[1];
    }
    ...
    if (this->GetFileDimensionality() >= 3) {
        streamStart = streamStart + (dataExtent[4] - this->DataExtent[4]) *
                                        this->DataIncrements[2];
    }
    ...
    if (this->File->fail()) {
        vtkErrorMacro(
            << "File operation failed: " << streamStart
            << ", ext: " << dataExtent[0] << ", " << dataExtent[1] << ", "
            << dataExtent[2] << ", " << dataExtent[3] << ", " << dataExtent[4]
            << ", " << dataExtent[5]);
        vtkErrorMacro(
            << "Header size: " << this->GetHeaderSize(idx) << ", file ext: "
...
void vtkOOCMetaImageReaderUpdate2(
    vtkOOCMetaImageReader* self, vtkImageData* data, IT* inPtr, OT* outPtr)
{
    ...
    streamRead =
        static_cast<unsigned long>(pixelRead * self->GetDataIncrements()[0]);
    streamSkip0 = (long)(self->GetDataIncrements()[1] - streamRead);
    streamSkip1 =
        (long)(self->GetDataIncrements()[2] - (dataExtent[3] - dataExtent[2] + 1) * self->GetDataIncrements()[1]);
    pixelSkip = data->GetNumberOfScalarComponents();

    // read from the bottom up
    if (!self->GetFileLowerLeft()) {
        streamSkip0 =
            (long)(-static_cast<long>(streamRead) - self->GetDataIncrements()[1]);
        streamSkip1 =
            (long)(self->GetDataIncrements()[2] + (dataExtent[3] - dataExtent[2] + 1) * self->GetDataIncrements()[1]);
    }
    ...
    target =
        (unsigned long)((dataExtent[5] - dataExtent[4] + 1) * (dataExtent[3] - dataExtent[2] + 1) / 50.0);
    target++;
    ...
    for (idx2 = dataExtent[4]; idx2 <= dataExtent[5]; ++idx2) {
        ...
        outPtr1 = outPtr2;
        for (idx1 = dataExtent[2]; !self->AbortExecute && idx1 <= dataExtent[3];
             ++idx1) {
            if (!(count % target)) {
...
#endif
            {
                vtkGenericWarningMacro(
                    ...
                    << idx1 << ", Tried to Read = " << streamRead << ", Read = "
                    << self->GetFile()->gcount() << ", Skip0 = " << streamSkip0
                    << ", Skip1 = " << streamSkip1 << ", FilePos = "
                    << static_cast<long long>(self->GetFile()->tellg()));
                delete[] buf;
                return;
            }
            // handle swapping
            if (self->GetSwapBytes()) {
                ...
                vtkByteSwap::SwapVoidRange(
                    buf, pixelRead * pixelSkip, sizeof(IT));
            }
            ...
            /* Unfortunately this doesn't work as a fix, but I'll leave it here
            for a bit to provoke ideas. if (filePos == -1)
            {
    ...
    ((defined(__sgi) && !defined(__GNUC__)) || (__BORLANDC__ >= 0x0560) || \
     defined(__APPLE_CC__))
            // this check is required for SGI's when vtk is build with
            // VTK_USE_ANSI_STDLIB seems that after a read that just reaches
            // EOF, tellg reports a -1. clear() does not work, so we have to
            // reopen the file. NB: Also Borland CBuilder 6 suffers from same
            // trouble As does Apple's gcc3
            if (filePos == -1) {
                self->OpenFile();
        ...
        // JF//VC : Modif
        // SetFilePointerEx(
        // self->GetFile(),static_cast<_int64>(self->GetFile()->tellg()) +
        // streamSkip1 + correction, 0, FILE_BEGIN);
        self->GetFile()->seekg(
            static_cast<long long>(self->GetFile()->tellg()) + streamSkip1 +
                correction,
            ios::beg);
        outPtr2 += outIncr[2];
    }
...
// templated to handle different data types.
template <class T>
void vtkOOCMetaImageReaderUpdate1(
    vtkOOCMetaImageReader* self, vtkImageData* data, T* inPtr)
{
    void* outPtr;
    ...
    switch (data->GetScalarType()) {
        vtkTemplateMacro(
            vtkOOCMetaImageReaderUpdate2(self, data, inPtr, (VTK_TT*)(outPtr)));
        default:
...
void vtkOOCMetaImageReader::ExecuteData(
    vtkDataObject* output, vtkInformation* outInfo)
{
    ...
    if (!this->FileName && !this->FilePattern) {
        vtkErrorMacro(
            "Either a valid FileName or FilePattern must be specified.");
        return;
    }
    ...
    vtkDebugMacro(
        "	ExecuteData, Reading extent: " << ext[0] << ", " << ext[1] << ", "
                                           << ext[2] << ", " << ext[3] << ", "
                                           << ext[4] << ", " << ext[5]);
    ...
    switch (this->GetDataScalarType()) {
        vtkTemplateMacro(
            vtkOOCMetaImageReaderUpdate1(this, data, (VTK_TT*)(ptr)));
        default:
...
void vtkOOCMetaImageReader::ComputeTransformedSpacing(double Spacing[3])
{
    if (!this->Transform) {
        ...
        this->Transform->TransformVector(
            transformedSpacing, transformedSpacing);

        for (int i = 0; i < 3; i++) {
            Spacing[i] = fabs(transformedSpacing[i]);
        }
        vtkDebugMacro(
            "Transformed Spacing " << Spacing[0] << ", " << Spacing[1] << ", "
                                   << Spacing[2]);
    }
}
...
void vtkOOCMetaImageReader::ComputeTransformedOrigin(double origin[3])
{
    if (!this->Transform) {
        ...
        vtkDebugMacro(
            "	ComputeTransformedOrigin, Transformed Origin "
            << origin[0] << ", " << origin[1] << ", " << origin[2]);
    }
}

void vtkOOCMetaImageReader::ComputeTransformedExtent(
    int inExtent[6], int outExtent[6])
{
    ...
    vtkDebugMacro(
        << "Transformed extent are:" << outExtent[0] << ", " << outExtent[1]
        << ", " << outExtent[2] << ", " << outExtent[3] << ", " << outExtent[4]
        << ", " << outExtent[5]);
}

void vtkOOCMetaImageReader::ComputeInverseTransformedExtent(
    int inExtent[6], int outExtent[6])
{
    double transformedExtent[3];
    int temp;
    ...
    if (!this->Transform) {
        ...
        transformedExtent[1] = inExtent[2];
        transformedExtent[2] = inExtent[4];
        this->Transform->GetLinearInverse()->TransformPoint(
    ...
    vtkDebugMacro(
        << "Inverse Transformed extent are:" << outExtent[0] << ", "
        << outExtent[1] << ", " << outExtent[2] << ", " << outExtent[3] << ", "
        << outExtent[4] << ", " << outExtent[5]);
}

void vtkOOCMetaImageReader::ComputeTransformedIncrements(
    vtkIdType inIncr[3], vtkIdType outIncr[3])
{
    ...
    if (!this->Transform) {
        ...
        vtkDebugMacro(
            << "Transformed Incr are:" << outIncr[0] << ", " << outIncr[1]
            << ", " << outIncr[2]);
    }
}

void vtkOOCMetaImageReader::ComputeInverseTransformedIncrements(
    vtkIdType inIncr[3], vtkIdType outIncr[3])
{
    double transformedIncr[3];
    ...
    if (!this->Transform) {
        ...
        transformedIncr[1] = inIncr[1];
        transformedIncr[2] = inIncr[2];
        this->Transform->GetLinearInverse()->TransformVector(
            transformedIncr, transformedIncr);
        outIncr[0] = (vtkIdType)transformedIncr[0];
        outIncr[1] = (vtkIdType)transformedIncr[1];
        outIncr[2] = (vtkIdType)transformedIncr[2];
        vtkDebugMacro(
            << "Inverse Transformed Incr are:" << outIncr[0] << ", "
            << outIncr[1] << ", " << outIncr[2]);
    }
}
